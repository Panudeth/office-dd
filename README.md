...
```bash
git clone https://github.com/Panudeth/office-dd.git
cd OfficeDD
npm install
npm run dev
```
...
```bash
docker run -p 3210:3210 --env-file .env ghcr.io/panudeth/office-dd:latest
...
**Vercel**: [Deploy to Vercel](https://vercel.com/new/clone?repository-url=https%3A%2F%2Fgithub.com%2FPanudeth%2Foffice-dd) — set the Supabase variables in the project settings. Note that webhooks/MCP/LINE need `SUPABASE_SECRET_KEY`, and local models are not reachable from Vercel.
