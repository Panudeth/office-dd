...
const dayLabel = (key: string) => {
  ...
  const th = d.toLocaleDateString('th-TH', { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' });
  return key === today ? `วันนี้ · ${th}` : key === yest ? `เมื่อวาน · ${th}` : th;
};
const timeLabel = (iso: string) => new Date(iso).toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' });
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      ...
      >
        <div className="flex flex-wrap items-center gap-2 text-[11px]">
          <span className="text-dim">ทั้งหมด <b className="text-parchment">{total}</b> เรื่อง</span>
          <span className="text-dim">ปรึกษาทีม <b className="text-brass">{escalatedCount}</b></span>
          {pending > 0 && <Badge variant="brass">กำลังตอบ {pending}</Badge>}
          <div className="ml-auto flex items-center gap-1.5">
            <div className="relative">
              <Search className="pointer-events-none absolute left-2 top-1/2 size-3 -translate-y-1/2 text-dim" />
              <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="ค้นคำถาม/คำตอบ" className="h-7 w-52 pl-6 text-[11px]" />
            </div>
            <Button size="sm" variant="outline" onClick={onRefresh} disabled={loading} title="โหลดใหม่">
        ...
        <div className="flex max-h-[62vh] min-h-0 flex-col gap-2 overflow-y-auto rounded-box border border-ink-600 bg-ink-900 p-2">
          {!byDay.length && (
            <Hint className="py-6 text-center">
              {loading ? 'กำลังโหลด...' : q ? 'ไม่พบรายการที่ค้น' : 'ยังไม่มีลูกค้าติดต่อเข้ามา - เมื่อมีคำถามผ่าน LINE หรือ token public จะขึ้นที่นี่'}
            </Hint>
          )}
          {byDay.map(([day, list]) => (
            <section key={day} className="flex flex-col gap-1">
              <h4 className="sticky top-0 z-10 flex items-center gap-2 bg-ink-900 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-dim">
                {dayLabel(day)} <span className="font-normal normal-case tracking-normal">· {list.length} เรื่อง</span>
              </h4>
              {list.map((m) => {
                ...
                return (
                  <article key={m.id} className={`rounded-box border-2 px-2 py-1.5 ${escalated ? 'border-brass/40 bg-ink-800' : 'border-ink-600 bg-ink-800'}`}>
                    <div className="mb-1 flex flex-wrap items-center gap-1.5 text-[10px]">
                      ...
                      {escalated && (
                        <span className="flex items-center gap-1 text-brass" title="PR ตอบเองไม่ได้ ต้องปรึกษาทีม">
                          <Users className="size-3" /> ปรึกษา {depts.map((d) => DEPT_BY_ID.get(d)?.shortTh ?? d).join(', ') || 'ทีม'}
                        </span>
                      )}
                    </div>
                    <div className="mb-1 rounded-box border border-wood-deep bg-wood-mid/60 px-2 py-1 text-[12px] text-parchment">
                      <span className="mr-1 text-[10px] text-parchment-2/70">ลูกค้าถาม</span>
                      {m.question}
                    </div>
                    <div className="rounded-box border border-ink-600 bg-ink-700 px-2 py-1 text-[12px] text-parchment-2">
                      <span className="mr-1 text-[10px] text-dim">ตอบลูกค้าว่า</span>
                      {m.customer_reply ? fmt(m.customer_reply) : <span className="text-dim">{m.status === 'running' ? 'กำลังตอบ...' : '(ยังไม่มีคำตอบ)'}</span>}
                    </div>
                    {(m.summary || m.minutes) && (
                      <button
                      ...
                      >
                        ...
                        สรุปภายใน (คนในเห็นเท่านั้น){escalated ? ' - จากที่ปรึกษาทีม' : ''}
                      </button>
                    )}
                    {on && (
                      <div className="mt-1 flex flex-col gap-1">
                        ...
                        {m.minutes && (
                          <div className="rounded-box border border-wood-dark bg-wood-deep/40 px-2 py-1 text-[11px] text-parchment-2">
                            <div className="mb-0.5 text-[10px] font-semibold text-brass-lite">รายงานการประชุม (เลขาฯ)</div>
                            {fmt(m.minutes)}
                          </div>
                        )}
                      </div>
